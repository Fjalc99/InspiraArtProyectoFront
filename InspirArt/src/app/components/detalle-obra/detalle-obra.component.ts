...
export class DetalleObraComponent {
  ...
  agregarComentarioUsuario() {
    ...
    this.comentariosService
      .crearComentarioUsuario(comentario as ComentarioDto)
      .subscribe({
        next: (nuevo) => {
          if (!this.obraSeleccionada.comentarios)
            this.obraSeleccionada.comentarios = [];
  ...
  guardarEdicionComentarioUsuario(comentario: ComentarioDto) {
    ...
    this.comentariosService
      .editarComentarioUsuario(comentario.idComentario, {
        ...comentario,
        comentario: this.nuevoComentario,
      })
      .subscribe({
        next: (editado) => {
          const idx = this.obraSeleccionada.comentarios.findIndex(
  ...
  eliminarComentarioUsuario(comentario: ComentarioDto) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.comentariosService
          .deleteComentarioUsuario(comentario.idComentario)
          .subscribe({
            next: () => {
              this.obraSeleccionada.comentarios =
                this.obraSeleccionada.comentarios.filter(
                  (c) => c.idComentario !== comentario.idComentario
                );
            },
          });
  ...
  enviarValoracion() {
    ...
    const onSuccess = (val: ValoracionDto) => {
      ...
      if (!this.obraSeleccionada.valoraciones)
        this.obraSeleccionada.valoraciones = [];
      const idx = this.obraSeleccionada.valoraciones.findIndex(
        (v) => v.id === val.id
      );
      if (idx > -1) {
    ...
    if (this.valoracionActual) {
      ...
      this.valoracionesService
        .editarValoracion(this.valoracionActual.id, valoracionEditada)
        .subscribe({
          next: onSuccess,
        });
    } else {
      ...
      this.valoracionesService
        .crearValoracion(nuevaValoracion as ValoracionDto)
        .subscribe({
          next: onSuccess,
        });
    }
  ...
  private actualizarMediaValoracionLocal() {
    if (
      this.obraSeleccionada.valoraciones &&
      this.obraSeleccionada.valoraciones.length > 0
    ) {
      const suma = this.obraSeleccionada.valoraciones.reduce(
        (acc, v) => acc + v.puntuacion,
        0
      );
      this.obraSeleccionada.mediaValoracion =
        suma / this.obraSeleccionada.valoraciones.length;
    } else {
  ...
  cargarValoracionUsuario() {
    ...
    this.valoracionesService.obtenerValoracionesUsuario().subscribe({
      next: (data) => {
        const valoracion = data.content.find(
          (v: ValoracionDto) => v.obraId === this.obraSeleccionada.idObra
        );
        if (valoracion) {
  ...
  esFavorito(): boolean {
    return this.favoritosService.favoritos.some(
      (f) => f.obra.idObra === this.obraSeleccionada.idObra
    );
  }

  toggleFavorito() {
    const fav = this.favoritosService.favoritos.find(
      (f) => f.obra.idObra === this.obraSeleccionada.idObra
    );
    if (fav) {
      this.favoritosService
        .eliminarFavorito(fav.obra.idObra)
        .subscribe(() => this.favoritosService.cargarFavoritos());
    } else {
      this.favoritosService
        .agregarFavorito(this.obraSeleccionada.idObra)
        .subscribe(() => this.favoritosService.cargarFavoritos());
    }
  }
